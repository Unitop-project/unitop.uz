...
### 1.A Dial Inference (design read в†’ dial values)
...
## 2. BRIEF в†’ DESIGN SYSTEM MAP

Once you have the design read (Section 0) and dials (Section 1), pick the right foundation. Do not invent CSS for things that have an official package. Do not pretend an aesthetic trend is an official system.

### 2.A When to reach for a real design system (use official packages)

| Brief reads asвЂ¦                         | Reach for                                                  | Why                                                           |
| ----------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------- |
...
LLMs default to clichГ©s. Override these defaults proactively. Each rule has a context-aware override path.
...
- **SERIF DISCIPLINE (VERY DISCOURAGED AS DEFAULT):**
  ...
  - For everything else (creative agency, design studio, modern brand, premium consumer, portfolio, lifestyle), **default sans-serif display** (Geist Display, ABC Diatype, SГ¶hne Breit, Cabinet Grotesk Display, Migra Sans, GT Walsheim, Inter Display, PP Neue Montreal). Sans display fonts are not "boring" вЂ” they are the default for the same reason black is the default in fashion.
...
- **BUTTON CONTRAST CHECK (mandatory, a11y):** Before shipping any button, verify the button text is readable against the button background. White button + white text, `bg-white` CTA with `text-white` label, transparent button against the page background with no border в†’ all banned. Audit every CTA: contrast ratio WCAG AA min (4.5:1 for body, 3:1 for large text 18px+). Same rule applies to ghost buttons over photographic backgrounds (use a backdrop, scrim, or stroke).
...
- **NO DUPLICATE CTA INTENT (mandatory):** Two CTAs with the same intent on one page is a Pre-Flight Fail. Examples of same intent: "Get in touch" + "Contact us" + "Let's talk" + "Start a project" + "Start something" + "Reach out" = all "contact" intent в†’ pick ONE label and use it everywhere on the page (nav, hero, footer). Same for "Try free" + "Get started" + "Sign up free" (all "signup" intent) and "View work" + "See selected work" + "Browse projects" (all "portfolio" intent). One label per intent.
- **FORM CONTRAST CHECK (mandatory, a11y):** Form inputs, placeholder text, focus rings, helper text, and error text all pass WCAG AA contrast against the section background. Light placeholders on a near-white form, white form on white page section, form labels grayer than 4.5:1 contrast в†’ all banned. Audit every form before shipping.
...
- **HERO TOP PADDING CAP (mandatory):** Hero top padding max `pt-24` (в‰€6rem) at desktop. More than that means the hero content floats halfway down the viewport and reads as a layout bug, not as intentional space. If your hero needs more breathing room, increase font scale or asset size, not top padding.
...
- **BENTO CELL COUNT RULE (mandatory):** A bento grid has EXACTLY as many cells as you have content for. 3 items в†’ 3 cells (1+2 split, or 2+1, or asymmetric trio). 5 items в†’ 5 cells (2+3, 3+2, hero+4, etc.). If your grid has an empty cell in the middle or at the end, you planned wrong. Re-shape the grid; do not paste a blank tile.
...
- **Default content shape per section:** short headline (в‰¤ 8 words) + short sub-paragraph (в‰¤ 25 words) + one visual asset OR one CTA. Anything more must be justified by the section's job.
...
- **Fake-precise numbers are flagged.** Numbers like `92%`, `4.1Г—`, `48k`, `5.8 mm`, `13.4 lb` either:
...
- **One copy register per page.** Don't mix technical mono ("47 tasks В· 0.6 ctx-switches/day"), editorial prose, and marketing punch in the same composition unless the brand voice explicitly calls for it.

### 4.10 Quotes & Testimonials

- **Max 3 lines** of quote body. Never 6. If the original quote is longer в†’ cut it. A landing-page quote is a snippet, not the full review.
...
- **NO generic names.** "John Doe", "Sarah Chan", "Jack Su" в†’ use creative, realistic, locale-appropriate names.
- **NO generic avatars.** No SVG "egg" or Lucide user icons в†’ use believable photo placeholders or specific styling.
- **NO fake-perfect numbers.** Avoid `99.99%`, `50%`, `1234567`. Use organic, messy data (`47.2%`, `+1 (312) 847-1928`).
- **NO startup-slop brand names.** "Acme", "Nexus", "SmartFlow", "Cloudly" в†’ invent contextual, premium names that sound real.
- **NO filler verbs.** "Elevate", "Seamless", "Unleash", "Next-Gen", "Revolutionize" в†’ concrete verbs only.
...
- **NO "Brand В· No. 01"-style sub-eyebrows.** "Marrow В· No. 01 В· The 6-quart" type micro-meta lines. Skip them.

**Section numbering & micro-labels**

- **NO section-number eyebrows.** `00 / INDEX`, `001 В· Capabilities`, `002 В· Featured commission`, `06 В· how it works`, `05 В· The honest table` - banned. Eyebrows should name the topic in plain language, not enumerate.
- **NO `01 / 4`-style pagination on images or bento tiles.** If the user can count, they don't need the label.
- **NO `Scroll В· 001 Capabilities`-style scroll cues.** A simple arrow or "Scroll" is enough; no section-number prefix.
- **NO "Index of Work, 2018 - 2026"-style range labels** as eyebrows. Just say what the section is.

**Separators & dots**

- **The middle-dot (`В·`) is rationed.** Maximum 1 per line in metadata strips. Do NOT use it as the default separator for everything ("foo В· bar В· baz В· qux В· quux"). If you need a separator family, prefer line breaks, hairlines, or columns.
...
- **NO em-dash (`вЂ”`) as a design element OR anywhere else.** See Section 9.G below for the complete, non-negotiable ban. The em-dash character is forbidden in headlines, eyebrows, pills, body copy, quotes, attribution, captions, button text, and alt text. Use the regular hyphen (`-`).
...
- **NO vertical rotated text** ("INDEX OF WORK, 2018 - 2026" rotated 90В°). Agency-portfolio clichГ©. Use it only when the brief is explicitly agency / Awwwards / experimental AND it serves a real composition purpose.
...
- **NO fake version footers** ("v0.6.2-rc.1", "last sync 4s ago В· main") inside fake screenshots. Adds nothing, screams AI.
...
- **NO weather / locale strips** ("LIS 14:23 В· 18В°C") in headers/footers unless the brief is explicitly about a place / time-zone-distributed studio.
...
- **NO pills/labels/tags overlaid on images.** No `<span>` overlays on photos with tags like `Brand В· 02`, `PLATE В· BRAND`, `Field notes - journal`. Either let the image speak alone, or add a caption directly below (outside the image).
- **NO photo-credit captions as decoration.** Strings like `Field study no. 12 В· Ines Caetano`, `Plate 03 В· House archive`, `Frame XII В· 35mm` under stock/picsum images are pretentious. Photo credit is allowed ONLY when there is a real photographer being credited for a real photo (with permission). Otherwise: skip the caption or use a one-line functional caption ("The 6-quart, in Sage.").
- **NO version footers on marketing pages.** Footer strings like `v1.4.2`, `Build 0048`, `last sync 4s ago В· main` are CLI / devtool fixtures, not landing-page content. Banned on marketing/landing/portfolio pages.
...
- **NO decoration text strip at hero bottom.** Patterns like `BRAND. MOTION. SPATIAL.`, `TYPE / FORM / MOTION`, `DESIGN В· BUILD В· SHIP`, `ESTD. 2018 В· LISBON В· BRAND. MOTION. SPATIAL.` as a small mono-caps strip across the bottom of the hero are an agency-portfolio clichГ©. Banned by default. Only acceptable when the strip carries real, navigable links (sticky bottom nav) or real status info (cookie banner, build info on a docs site).
...
- **Locale / city-name / time / weather strips are banned for 99% of briefs.** "Lisbon, working with founders" in the hero, "1200-690 Lisbon, Portugal" in the footer, "Lisbon 14:23 В· 18В°C" in the nav. These are agency-portfolio decoration tells. Allowed ONLY when: the brief explicitly describes a globally-distributed studio with timezone-relevant work, OR a travel-focused brand, OR a real-world physical venue. A single contact-address mention in the footer is fine; an atmospheric locale strip is not.
- **Scroll cues are banned.** `Scroll`, `в†“ scroll`, `Scroll to explore`, `Scroll to walk through it`, animated mouse-wheel icons. If the user has not scrolled yet, they are looking at the hero. They know what scroll is. The bottom of the viewport does not need a label.
...
**Em-dash (`вЂ”`) is COMPLETELY banned.** It is the LLM's signature stylistic crutch and it is the #1 visual Tell in production tests. There is no "limited use" allowance, no "natural language frequency" allowance, no "in body copy is fine" allowance. None.
...
- **Banned in en-dash form too (`вЂ“`) when used as a separator.** Date ranges (`2018-2026`) use a hyphen. Number ranges (`в‚¬40-80k`) use a hyphen.

The ONLY permitted dash characters on the page are:

- Regular hyphen `-` (for compound words, ranges, line dividers in markup)
- Minus sign in math (`-5В°C`)

If your output contains a single `вЂ”` or `вЂ“` anywhere visible to the user, the output fails the Pre-Flight Check and must be rewritten.
...
- **Horizontal Scroll Hijack** - Vertical scroll в†’ horizontal pan.
...
- **Preserve copy voice** unless asked for a rewrite. Visual modernisation в‰  content rewrite.
...
- IA, content, and SEO sound в†’ **targeted evolution** (Levers 1-4). ~70% of value at ~40% of risk.
- Visual debt is structural (broken IA, no design system, broken mobile) в†’ **full redesign** with strict content preservation.
- Brand itself is changing в†’ **greenfield**.
...
- [ ] **ZERO em-dashes (`вЂ”`) anywhere on the page.** Headlines, eyebrows, pills, body, quotes, attribution, captions, buttons, alt text. Zero. (Section 9.G - non-negotiable.)
...
- [ ] **Hero fits the viewport**: headline в‰¤ 2 lines, subtext в‰¤ 20 words AND в‰¤ 4 lines, CTA visible without scroll, font scale planned around image?
...
- [ ] **EYEBROW COUNT (mechanical)**: count instances of `uppercase tracking` micro-labels above section headlines across all components. Count в‰¤ ceil(sectionCount / 3)? Hero counts as 1.
...
- [ ] **Navigation on ONE line** at desktop, height в‰¤ 80px?
- [ ] **Section-Layout-Repetition** check: no two sections share the same layout family (at least 4 different families across 8 sections)?
- [ ] **Bento has rhythm AND exact cell count** (N items в†’ N cells, no empty cells in middle or at end)?
...
- [ ] **No pills/labels overlaid on images** (no `Plate В· Brand`, no `Field notes - journal`)?
- [ ] **No photo-credit captions as decoration** (`Field study no. 12 В· Ines Caetano`)?
...
- [ ] **No scroll cues** (`Scroll`, `в†“ scroll`, `Scroll to explore`)?
- [ ] **No version labels in hero** (V0.6, BETA, INVITE-ONLY) unless the brief is a launch?
- [ ] **No section-numbering eyebrows** (`00 / INDEX`, `001 В· Capabilities`, `06 В· how it works`)?
...
- [ ] **Content density** sane: no 20-row data tables, no fake-precise specs without justification, в‰¤ 25-word sub-paragraphs by default?
- [ ] **Quotes в‰¤ 3 lines** of body, attribution clean (no em-dash)?
...
- Apple Human Interface Guidelines в†’ Materials
- Apple Developer Documentation в†’ Liquid Glass
- Apple Developer Documentation в†’ Adopting Liquid Glass
- SwiftUI в†’ Material
